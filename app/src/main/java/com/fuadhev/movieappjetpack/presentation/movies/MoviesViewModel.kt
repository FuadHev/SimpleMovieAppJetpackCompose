...
import com.fuadhev.movieappjetpack.domain.usecase.GetMovieUseCase
...
class MoviesViewModel @Inject constructor(private val getMovieUseCase: GetMovieUseCase) :
    ...
    private fun getMovies(search: String) {
        job?.cancel()
        job = getMovieUseCase.getMovies(search).onEach {
            when (it) {
                is Resource.Success -> {
                ...
                is Resource.Error -> {
                    _state.value = MovieState(error = it.exception ?: "Error!")
                }
